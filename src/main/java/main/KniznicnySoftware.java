...
public class KniznicnySoftware {
	...
	private final int POCET_KNIH_NA_VYPIS_PRI_NENAJDENI = 5;
	 ...
	 * 
	 * @param IDknihy
	 *            jedineèné èíslo vıtlaèku knihy
	 * @param NazovPobocky
	 *            názov poboèky v ktorej sa kniha nachádza
	 ...
	 * 
	 * @param NazovKnihy
	 *            nazov knihy ktorú chceme vyh¾ada
	 * @param NazovPobocky
	 *            nazov pobocky na ktorej sa bude vyh¾adáva
	 * @return zoznam kníh na tejto poboèke so zadanım názvom, alebo nasledovné
	...
	public List<String> najdiKnihyPodlaNazvu(String NazovKnihy, String NazovPobocky) {
		...
		List<Kniha> knihy = pobocka.najdiKnihyPodlaNazvu(NazovKnihy, POCET_KNIH_NA_VYPIS_PRI_NENAJDENI);
	 ...
	 * 
	 * @param nazovPobocky
	 *            nazov poboèky z ktorej sa poièiava
	 * @param IDknihy
	 *            id vıtlaèku knihy ktorá sa má poièa
	 * @param cisloPreukazu
	 *            èíslo preukazu èitate¾a
	...
	public String zapozicajKnihu(String nazovPobocky, String IDknihy, String cisloPreukazu) {
		...
		new Vypozicka(pobocka, kniha, citatel, aktualnyDatum.plus(kniha.getVypozicnaDoba()), aktualnyDatum);
		return "kniha bola zapoièaná";
	}

	/**
	 * Zapoièia knihu dannému èitate¾ovi - pouívaná len pri naèítavní databázy
	 * zo súboru
	 * 
	 * @param nazovPobocky
	 *            nazov poboèky z ktorej sa poièiava
	 ...
	 *            èíslo preukazu èitate¾a
	 * @param datumPozickyS
	 *            dátum kedy sa uskutoènila vıpoièka
	 */
	private void zapozicajKnihu(String nazovPobocky, String IDknihy, String cisloPreukazu, String datumPozickyS) {
		...
		if (kniha == null) {
			System.out.println("je tonull");
		}
		new Vypozicka(pobocka, kniha, citatel, datumPozicky.plus(kniha.getVypozicnaDoba()), datumPozicky);
	}
	 ...
	 *            èíslo preukazu èitate¾a
	 * @param IDKnihy
	 *            id vıtlaèku knihy ktorá sa má vráti
	 * @param nazovPobocky
	 *            nazov poboèky do ktorej èitate¾ vracia
	 ...
	 * @param IDKnihy
	 *            id vıtlaèku knihy ktorá sa má vráti
	 * @param nazovPobockyKdejeNaozaj
	 *            nazov poboèky kde sa kniha naozaj nachadza
	 * @param nazovPobockyKdeBolaVratena
	 *            nazov poboèky do ktorej bola kniha vratena
	 * @param datumVrateniaS
	 *            dátum kedy bola kniha vrátená
	 */
	private void vratKnihu(String IDCitatela, String IDKnihy, String nazovPobockyKdejeNaozaj,
			String nazovPobockyKdeBolaVratena, String datumVrateniaS) {
		boolean vratenieNaInejPobocke = false;
		LocalDate datumVratenia = LocalDate.parse(datumVrateniaS);
		Pobocka pobockaDoKtorejSaVracia = pobockyPodlaMena.find(nazovPobockyKdejeNaozaj);
		Citatel citatel = citateliaPodlaID.find(Integer.parseInt(IDCitatela));
		Vypozicka vypozicka = citatel.getVypozickuPodlaID(Integer.parseInt(IDKnihy));
		...
		vypozicka.setNazovPobockyKdeBolaVratena(nazovPobockyKdeBolaVratena);
		vypozicka.getPobocka().vymazVypozicku(vypozicka);
		vypozicka.getKniha().setAktualnaVypozicka(null);
		citatel.vymazVypozicku(vypozicka);
	 ...
	 * 
	 * @param nazov
	 *            nazov poboèky z ktroej chceme vıpis
	 ...
	 * 
	 * @param nazovPobocky
	 *            názov poboèky z ktorej chceme vıpis
	 * @return list stringov obsahujúci informácie o knihách, pri ktorıch
	 *         èitatelia meškajú s vrátením
	 */
	...
	/**
	 * získa informácie o vıpoièkách èitate¾a ktoré boli odovzdané neskoro v
	 * dannom èasovom rozmedzí
	 * 
	 * @param idCitatela
	 *            èíslo preukazu èitate¾a
	 * @param datumOds
	 *            dátum od kedy chceme vıpis pôièiek
	 * @param datumDos
	 *            dátum do kedy chceme vıpis pôièiek
	 * @return informácie o vıpoièkách èitate¾a ktoré boli odovzdané neskoro v
	 *         dannom èasovom rozmedzí
	 */
	public List<String> getinfoOOmesaknychVypozickachCitatela(String idCitatela, String datumOds, String datumDos) {
		...
		for (Vypozicka vypozicka : citatel.oneskoreneVypozicky(datumOd, datumDo)) {
			info.add("kniha " + vypozicka.getNazovKnihy() + " bola vratena "
					+ vypozicka.getDatumKedySaVratila().format(formatter) + " vratit sa mala najneskor "
					+ vypozicka.getDatumDoKedySaMaVratit().format(formatter) + " to je "
					+ vypozicka.getPocetDniMeskania() + " dni meskania");
		}
		return info;
	...
	public List<String> getHistoriaVypoziciekCitatela(String idCitatela) {
		...
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd. MM. yyyy");
		for (Vypozicka vypozicka : citatel.getKnihyPozicaneVMinulosti()) {
			info.add(vypozicka.getNazovKnihy()+ " " +vypozicka.getDatumZapozicania().format(formatter)+
	 ...
	 * 
	 * @param pobockaZs
	 *            poboèka z ktorej presúvame agendu, a ktorá bude vymazaná
	 * @param pobockaDos
	 *            poboèka do ktorej presúvame agendu
	 ...
	 * 
	 * @param id
	 *            èíslo preukazu èitate¾a ktorého chceme vymaza
	 ...
	 * 
	 * @param IDpreukazuCitatela
	 *            èíslo preukazu èitate¾a ktorého pôièky chceme získa
	 * @return list stringov obsahujúci informácie o momentálne poièanıch
	 *         knihách
	 */
	 ...
	 * 
	 * @param nazovPobocky
	 *            názov poboèky z ktorej chceme vymaza knihu
	 * @param IDKnihy
	 *            id vıtlaèku knihy ktorú chceme vyradi
	 ...
	 * 
	 * @param nazovPobocky
	 *            názov poboèky z ktorej chceme urobi vıpis
	 ...
	 * 
	 * @param meno
	 *            meno nového èitate¾a
	 * @param priezvisko
	 *            priezvisko nového èitate¾a
	 */
	...
	/**
	 * pridá do systému nového èitate¾a - pouíva sa pri naèítavaní databázy zo
	 * systému
	 * 
	 * @param citatel
	 *            objekt reprezentujúci èitate¾a
	 */
	private void pridajCitatela(Citatel citatel) {
		citateliaPodlaID.insert(citatel.getCisloPreukazu(), citatel);
		SplayTree<Integer, Citatel> citateliaPodlanovehoMena = citateliaPodlaMena
				.find(citatel.getPriezvisko() + citatel.getMeno());
		if (citateliaPodlanovehoMena == null) {
			citateliaPodlanovehoMena = new SplayTree<>();
	 ...
	 * 
	 * @return list stringov obsahujúci mená èitate¾ov spolu s ich èíslamy
	 *         èitate¾skıch preukazov
	 */
	 ...
	 * 
	 * @param nazovPobocky
	 *            názov poboèky do ktorej sa pridá táto kniha
	 * @param nazov
	 *            názov knihy
	 * @param autor
	 *            autor knihy
	 * @param iSBN
	 *            isbn èíslo tejto knihy
	 * @param eAN
	 *            ean èíslo tejto knihy
	 * @param zaner
	 *            zaner tejto knihy
	 * @param poplatokZaDenOmeskania
	 *            poplatok za deò omeškania pri vrátení knihy
	 * @param vypozicnaDoba
	 *            vıpoièná doba tejto knihy
	 ...
	 * 
	 * @param IDpreukazu
	 *            èíslo preukazu èitate¾a ktorého meno chceme zisti
	 ...
	 * 
	 * @param datum
	 *            na ktorı sa má nastavi kninica
	 */
	 ...
	 * 
	 * @param pocetPismenok
	 *            dåka textového reazca ktorı sa má vygenerova
	 * @param rnd
	 ...
	 * 
	 * @param pocetUzivs
	 *            poèet uívate¾ov ktorı sa majú vygenerova
	 * @param pocetPobocieks
	 *            poèet poboèiek ktoré sa majú vygenerova
	 * @param pocetKnihVkazdejPobockes
	 *            poèet kníh v kadej poboèke ktoré sa majú vygenerova
	 */
	public void generuj(String pocetUzivs, String pocetPobocieks, String pocetKnihVkazdejPobockes) {
		...
		for (int i = 0; i < pocetPobociek; i++) {
			...
			for (int j = 0; j < pocetKnihNaPobocku; j++) {
				...
				pridajKnihu(menoPobocky, meno, priezvisko, Isbn, Isbn, zaner, String.valueOf(rnd.nextInt(5)),
						String.valueOf(rnd.nextInt(60)));
			}
		}
	}

	/**
	 * uloí celú databázu do súboru - pomalá ba i nefunkèná funkcia
	 * 
	 * @param nazovSuboru
	 *            názov súboru do ktorého sa uloia dáta
	 * @return správu o vısledku úspešnosti ukladania údajov do súboru
	 */
	public String ulozDoSuboruS(String nazovSuboru) {
		...
		String vsetkyUdaje = pocetCitatelov + "," + Citatel.getNextCisloPreukazu() + System.lineSeparator()
				+ vsetciCitateliaString + pocetPobociek + System.lineSeparator() + vsetkyPobockyString + pocetKnih + ","
				+ Kniha.getNextID() + System.lineSeparator() + vsetkyKnihyString + pocetVypoziciek
				+ System.lineSeparator() + vsetkyVypozickyString;
		try (FileWriter fw = new FileWriter(nazovSuboru, true);
				BufferedWriter bw = new BufferedWriter(fw);
				PrintWriter out = new PrintWriter(bw)) {
			out.println(vsetkyUdaje);
			return "udaje boli zapisane";
		} catch (IOException e) {
			return "udaje neboli zapisane";
		}
	...
	public String ulozDoSuboru(String nazovSuboru) {
		...
		int pocetVypoziciek;
		int pocetCitatelov = citateliaPodlaID.getSize();
		LinkedList<Citatel> citatelia = new LinkedList<>();
		SplayTree<Integer, Kniha> knihy = new SplayTree<>();
		SplayTree<String, String> pobocky = new SplayTree<>();
		List<Vypozicka> vypozicky = new LinkedList<>();
		for (Citatel citatel : citateliaPodlaID.toListLevelOrder()) {
			citatelia.add(citatel);
			for (Vypozicka vypozicka : citatel.getVypozickyVMinulosti()) {
				pobocky.insert(vypozicka.getNazovPobocky(), vypozicka.getNazovPobocky());
				knihy.insert(vypozicka.getIDKnihy(), vypozicka.getKniha());
				vypozicky.add(vypozicka);
			}
			for (Vypozicka vypozicka : citatel.getAktualneVypozicky()) {
		...
		for (Pobocka pobocka : pobockyPodlaMena.toListLevelOrder()) {
			pobocky.insert(pobocka.getNazov(), pobocka.getNazov());
			for (Kniha k : pobocka.getVsetkyKnihyNaPobocke()) {
				knihy.insert(k.getID(), k);
			}
		}
		...
		pocetPobociek = pobocky.getSize();
		pocetVypoziciek = vypozicky.size();
		try (FileWriter fw = new FileWriter(nazovSuboru, false);
				BufferedWriter bw = new BufferedWriter(fw);
				PrintWriter out = new PrintWriter(bw)) {
			out.println(pocetCitatelov + "," + Citatel.getNextCisloPreukazu());
			for (Citatel citatel : citatelia) {
				out.println(citatel.getSuboroveUdaje());
			}
			out.println(pocetPobociek);
			for (String nazovPobocky : pobocky.toListLevelOrder()) {
				if (pobockyPodlaMena.find(nazovPobocky) == null) {
					out.println(nazovPobocky + ",n");
				} else {
					out.println(nazovPobocky + ",e");
					;
				}
			}
			out.println(pocetKnih + "," + Kniha.getNextID());
			for (Kniha kniha : knihy.toListLevelOrder()) {
				if (kniha.bolaVymazana()) {
					out.println(kniha.getSuboroveUdaje() + ",n");
				} else {
					out.println(kniha.getSuboroveUdaje() + ",e");
				}
			}
			out.println(pocetVypoziciek);
			for (Vypozicka vypozicka : vypozicky) {
				out.println(vypozicka.getSuboroveUdaje());
			}
			return "udaje boli zapisane";
		} catch (IOException e) {
			return "udaje neboli zapisane";
	 ...
	 * 
	 * @param nazovSuboru
	 *            názov súboru z ktorého sa naèítajú dáta
	...
	public String nacitajZoSuboru(String nazovSuboru) {
		try {
			...
			for (int i = 0; i < pocetKnih; i++) {
				...
				Kniha kniha = new Kniha(udaje[0], udaje[1], udaje[2], udaje[3], udaje[4], pobocka,
						Double.parseDouble(udaje[6]), Period.ZERO.plusDays(Integer.parseInt(udaje[7])),
			...
			LinkedList<String[]> udajeOvratenychVypozickach = new LinkedList<>();
			LinkedList<String[]> udajeOnevratenychVypozickach = new LinkedList<>();
			...
			for (String[] u : udajeOvratenychVypozickach) {
				this.zapozicajKnihu(u[7], u[1], u[2], u[3]);
				this.vratKnihu(u[2], u[1], u[7], u[6], u[5]);
			}
